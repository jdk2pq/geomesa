...
class RasterStoreTest extends Specification {
  ...
  def getRasterStore = RasterStore("user", "pass", "testInstance", "zk", s"wcs_mosaic_test_${Random.alphanumeric.take(5).mkString}", "SUSA", "SUSA", useMock = true)
  ...
  def generateQuery(envelope: Envelope) = {
    val fauxRes = "10" //change this once the default value changes in getRow() in AccumuloBackedRasterOperations
    val bb = BoundingBox(envelope)
    new RasterQuery(bb, fauxRes, None, None)
  }
  ...
  def getTestRasters(width: Int, height: Int, bbox: BoundingBox): List[Raster] = {
    ...
    rasterList += Raster(grayImage, metadataTR)
    rasterList += Raster(whiteImage, metadataTL)
    rasterList += Raster(grayImage, metadataBL)
    rasterList += Raster(whiteImage, metadataBR)
    rasterList.toList
  }

  def getImageByteArray(rasterList: List[Raster], width: Int, height: Int, envelope: ReferencedEnvelope, resX: Double, resY: Double): Array[Byte] = {
    val image = RasterUtils.mosaicRasters(rasterList, width*2, height*2, envelope, resX, resY)
    val baos = new ByteArrayOutputStream()
//    ImageIO.write(image, "png", new File(s"${Random.alphanumeric.take(5).mkString}_$extension.png"))
    ImageIO.write(image, "png", baos)
    baos.toByteArray
  }

  def runMosaicTest(width: Int , height: Int, bbox: BoundingBox) = {
  ...
  "RasterStore" should {
    "mosaic Rasters appropriately for 4 10x10 Rasters" in {
      runMosaicTest(10, 10, BoundingBox(-45.0001, 45.0001, -45.0001, 45.0001))
    }

    "mosaic Rasters appropriately for 4 100x100 Rasters" in {
      runMosaicTest(100, 100, BoundingBox(-135.0001, -44.9999, -45.0001, 45.0001))
    }

    "mosaic Rasters appropriately for 4 1000x1000 Rasters" in {
      runMosaicTest(1000, 1000, BoundingBox(-135.0001, -44.9999, -45.0001, 45.0001))
    }
  }
}
